...
const DEBUG = {
    ...
    variables: {
        ...
        starCount: { value: 5000, min: 0, max: 10000, step: 100, label: 'Star Count' },
    ...
    toggles: {
        ...
        showLogs: { value: false, label: 'Show Debug Logs' },
...
function createStars() {
    ...
    const starCount = DEBUG.variables.starCount.value;
    ...
    for (let i = 0; i < starCount; i++) {
        const radius = DEBUG.variables.starDistance.value + Math.random() * 200;
        const phi = Math.random() * Math.PI * 2;
        const theta = Math.random() * Math.PI * DEBUG.variables.starSpread.value;
        ...
        twinkleSpeeds[i] = DEBUG.variables.starTwinkleSpeed.value * (0.5 + Math.random());
...
function animateRoad() {
    ...
    roadSegments.forEach(segment => {
        ...
        if (roadEndPosition < camera.position.z + 200) { // Keep 200 units of road ahead at all times
            segment.position.z = camera.position.z - roadLength * segmentCount;
        }
    });
...
function animateSpecks() {
    const dayFactor = Math.max(0, Math.sin(-dayNightCycle + Math.PI));
    const currentState = sceneStates[scenes[currentSceneIndex]];
    const roadVisible = DEBUG.toggles.showRoad.value;
    ...
    specks.forEach(speck => {
        ...
        if (speck.position.z > camera.position.z) {
            ...
            // Keep specks away from visible road
            if (roadVisible) {
                const minDistance = roadWidth / 2 + 5;
                const side = Math.random() < 0.5 ? -1 : 1;
                speck.position.x = side * (minDistance + Math.random() * 200);
            } else {
                speck.position.x = (Math.random() - 0.5) * 400;
            }
        }
        ...
        if (isSceneTransitioning) {
            ...
            // Get base colors for current and next scene
            const currentBaseColor = currentState.dayColors.ground.clone();
            const nextBaseColor = nextState.dayColors.ground.clone();
            
            // Add slight variation to colors
            const variation = 0.2;
            currentBaseColor.multiplyScalar(0.8 + Math.random() * variation);
            nextBaseColor.multiplyScalar(0.8 + Math.random() * variation);
            
            // Smoothly interpolate between colors
            const t = easeInOutCubic(sceneTransitionProgress);
            speck.material.color.copy(currentBaseColor).lerp(nextBaseColor, t);
        } else {
...
// Add a function to update stars when settings change
function updateStars() {
    createStars();
    log('Stars updated with new settings', 'info');
}
...
function createVisibilitySection() {
    ...
    const mainToggles = ['showRoad', 'showStreetLamps', 'showBuildings', 'showMountains', 'showSceneObjects'];
